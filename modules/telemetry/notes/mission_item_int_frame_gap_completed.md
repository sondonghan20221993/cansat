...
- [x] `SendMissionItemInt`에 `SendMissionItem`(legacy)과 동일한 GLOBAL_RELATIVE_ALT +
      ...
      UT 빌드 회귀 없음 (mavlink_bridge_app UT 전체: utils 118, main 14, cmds 4, dispatch 26 PASS).
- [x] `SendMissionItemInt`를 직접 겨냥한 UT 추가 (2026-07-13) — `RefLatE7`/`RefLonE7`가
      static이라 GLOBAL_POSITION_INT 프레임을 먼저 주입해 실제 파서로 설정하고,
      socketpair(단방향 pipe 대신 — 같은 fd로 write까지 캡처 가능)로 MISSION_REQUEST_INT를
      주입해 실제로 write()된 MISSION_ITEM_INT 프레임을 캡처·검증. frame byte(=3
      GLOBAL_RELATIVE_ALT), lat/lon degE7(legacy와 동일 공식으로 기대값 재계산 후 비교),
      고도 무변환, cmd/seq 필드까지 13개 assertion 전부 PASS.
