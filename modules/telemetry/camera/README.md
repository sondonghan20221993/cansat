...
- **시각 매칭 방식 채택**: 영상의 시작 절대시각과 openMCT 텔레메트리 CSV의
  ...
  **영상 시작 시각 소스는 2단계**(2026-07-16 개선, `mp4` 로컬 테스트로 양쪽 경로 검증):
  1. mp4 컨테이너의 `creation_time` 메타데이터(ffprobe) — 있으면 우선 사용.
     **실측 결과(2026-07-16): fpv4win은 이 태그를 기록하지 않음** → 실전에선 2번 경로 사용.
  2. 없으면 파일명(`<epoch_ms>.mp4`) 파싱으로 폴백 — 녹화 버튼 누른 시점의 PC 시계
     (버튼→실제 녹화 시작 지연만큼 오차 가능, 기존 방식).

  실행 시 어느 소스를 썼는지 콘솔에 출력됨(`시작 시각 소스: ...`).

  **실전 end-to-end 검증 완료 (2026-07-16)**: 실카메라 fpv4win 녹화(295.8초,
  `1784174893671.mp4`) × 실LoRa 텔레메트리 CSV(`telemetry_20260716_130801.csv`)
  매칭 → 1479행, `video_offset_sec` 0.19~295.6초 전 구간 정상.
