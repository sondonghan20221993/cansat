...
- [ ] **업링크 방향은 범위 밖으로 남음**: 지상이 아는 `boot_count`를
      UP2 프레임에도 실어 기체가 **이전 세션 번호를 단 명령을 거부**하는
      것(예전 세션 캡처 명령의 재생을 영구 차단하는 보안 강화)은 아직
      미구현 — 다운링크 반쪽만 완료된 상태
...
| **T2** | `073a680`(RunTx UFB 리셋 제거) 유지/롤백 판단 | 긴급-1 파생 | 기체(`lora_tdm_app`) | 없음 | T1 판단과 연동 | ⏸ BL-02, 판단 보류(T3로 대부분 해소) |
...
| **T5** | 부트 카운터 도입<br>(영속 저장 + 다운링크 + 업링크 태그) | ⓐⓑⓒ 보안·구분 | 기체+지상 | **DL2/UP +2B** | **T3**, cFE 리셋카운터 대체 가능성 확인 | 🟡 **다운링크만 완료**(BL-12/BL-03), 업링크 태그(재생 차단)는 미착수 |
| **T6** | seq 비교를 모듈러 윈도우로<br>`diff=(uint16)(seq-last); 0<diff<0x8000` | 문제 3 (wrap) | 기체 | 없음 | uint16/uint32 타입 정리 | ✅ 완료(BL-13) |
| **T7** | 대상앱→`uplink_app` 실행결과 회신 채널 + `EXECUTED` 코드 | 문제 5 | 기체 전반 | 내부 MID 신설 | 설계 필요(범위 큼) | ⏸ BL-08, 보류(설계 필요) |
| **T8** | UFB 코드표 전체 확정<br>(현재 미매핑 7종 포함) | A-2 | 기체+지상 | 코드값 확정 | **T7** | ⏸ BL-11, T7 종속 |
| **T9** | 재전송 인덱스를 `Flags` 여유비트에 | RF 진단 | 기체+지상 | 없음(여유비트) | 선택, T3와 동시 권장 | 🟡 BL-14, 미착수(선택) |
| **T10** | `LoadState()` 실패 경로 이벤트 추가 | 조용한 실패 | 기체 | 없음 | 없음 | 🤔 BL-17, 고민 필요(첫부팅 vs 손상 구분) |
