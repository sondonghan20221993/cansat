...
- **MODIFY(index)가 `ActiveResumeIndex`를 대상으로 할 때(현재 향하는 지점의 좌표를 바꿔 급각도
  진로변경이 발생하는 경우)도 허용한다** — DELETE와 달리 차단하지 않음, 장애물 회피 등 의도된
  긴급 수정 용도로 사용자 책임 하에 허용.
- **index 동기화 전제(2026-07-25 확정)**: 이 시스템은 단일 운용자·단일 GUI 세션 전제라, 지상국의
...
없었다. 이륙 절차는 여전히 이 시스템의 route 관리 범위 밖. (2-pass 보정 설계는 2026-07-26
MAD 잔차 임계값 확정으로 완결됐고 openMCT에 구현까지 완료됨(BL-58) — 아래 §18.4.6.2.1은
더 이상 보류 상태 아님.)
...
등 GPS 항법 시스템의 표준 패턴과 동일). 이 로컬 좌표계의 원점은 **lap 1 시작 시점 기체 GPS를
라이브로 스냅샷하지 않고, 사전에 계획된 원형 경로(lap 1 계획 waypoint) 자체의 값**(계획
waypoint[0] 또는 계획 원피팅 중심 `cx_plan/cy_plan`)을 그대로 쓴다(2026-07-25 정정 — 라이브
GPS 스냅샷은 그 순간의 GPS 노이즈·수렴 상태·fix quality에 좌표계 전체가 좌우되는 위험이 있어
기각). 계획 경로는 설계 시점에 이미 확정된 값(BL-57 지도 입력 시 절대좌표로 저장)이라 노이즈나
캡처 타이밍 문제가 없다. `mavlink_bridge_app`의 `RefLatE7`/`RefLonE7`(매 `GLOBAL_POSITION_INT`마다
갱신되는 현재 위치, §18.4.6.2 참조)은 이 계산에 **사용 금지** — 대신 lap1 실측 GPS 샘플들을
계획 경로 기준점으로 변환해 누적하고, 그 기준점이 실측 원피팅과 계획 원피팅 모두에 공통 적용되므로
두 원이 같은 좌표계에서 자연히 비교 가능하다.
...
**상태머신 위치 (2026-07-25 확정)**: 2-pass 상태머신(IDLE/LAP1_ACTIVE/CORRECTING/LAP2_ACTIVE/DONE)과
샘플 누적은 브라우저 GUI가 아니라 **지상 백엔드**(`fc_serial_ws_server.py`, 이미 다운링크 수신·
`telemetry_logs` 기록을 상시 수행 중인 프로세스)가 보관한다 — GUI 탭을 새로고침/종료해도 비행 중인
2-pass 진행 상황이 유지되고, GUI는 상태를 조회만 한다.

**진행 중 수동 개입(2026-07-25 확정)**: LAP1_ACTIVE~LAP2_ACTIVE 구간에도 사용자가 수동으로
ADD/DELETE/MODIFY/REPLACE를 보내는 것을 차단하지 않는다 — 다만 GUI가 "2-pass 진행 중입니다,
그래도 보내시겠습니까?" 재확인 다이얼로그를 띄운 뒤 전송(REPLACE 재확인과 동일 패턴). 자동
시퀀스가 예기치 못한 경로 변경을 보고 오작동할 위험은 사용자가 감수.
