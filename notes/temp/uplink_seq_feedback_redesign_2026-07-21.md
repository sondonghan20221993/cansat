...
- [ ] 업링크 프레임에도 지상이 아는 `boot_count`를 실어 기체가
      **이전 세션 번호를 단 명령을 거부** → 예전 세션에서 캡처한 명령의
      재생이 영구 불가해짐 (보안 향상)
- [ ] `(boot_count, seq)` 복합 식별자가 되면 seq는 세션마다 리셋돼도
      안전 → **문제 3(65535 wrap)의 심각도가 크게 낮아짐**
- [ ] 저장 위치: 현재 구조가
      `UPLINK_APP_PersistentState_t { Magic, LastAcceptedSequence, Checksum }`,
      `Checksum = Magic + LastAcceptedSequence`.
      `BootCount` 추가 시 체크섬 식이 바뀌어 **기존 파일은 자동 검증 실패
      → 신규 상태로 시작**(무해한 마이그레이션이나 의도임을 문서화할 것)
- [ ] **대안 먼저 확인**: cFE가 관리하는 프로세서 리셋 카운터
      (`CFE_ES` 리셋 정보)로 대체 가능한지 — 가능하면 커스텀 필드 불필요
...
| # | Task | 해결 대상 | 수정 범위 | 프로토콜 변경 | 선행조건 |
|---|---|---|---|---|---|
| **T1** | 재전송 중복을 실패로 보고하지 않기<br>(`seq == last` → `DUPLICATE`, `seq < last` → replay) | 긴급-1 | 기체(`uplink_app`) | 없음 | 없음 |
| **T2** | `073a680`(RunTx UFB 리셋 제거) 유지/롤백 판단 | 긴급-1 파생 | 기체(`lora_tdm_app`) | 없음 | T1 판단과 연동 |
| **T3** | 다운링크에 `ufb_seq` 동봉<br>(`UPLINK_STATUS_MID`의 `LastCommandSequence`를 그대로) | 문제 4 (+A-1 소멸) | 기체+지상 | **DL2 +2B** | air time 마진 확인 |
| **T4** | 지상이 다운링크 seq 보고 자가복구 | 긴급-2 | 지상 | 없음(T3 재사용) | **T3** |
| **T5** | 부트 카운터 도입<br>(영속 저장 + 다운링크 + 업링크 태그) | ⓐⓑⓒ 보안·구분 | 기체+지상 | **DL2/UP +2B** | **T3**, cFE 리셋카운터 대체 가능성 확인 |
| **T6** | seq 비교를 모듈러 윈도우로<br>`diff=(uint16)(seq-last); 0<diff<0x8000` | 문제 3 (wrap) | 기체 | 없음 | uint16/uint32 타입 정리 |
| **T7** | 대상앱→`uplink_app` 실행결과 회신 채널 + `EXECUTED` 코드 | 문제 5 | 기체 전반 | 내부 MID 신설 | 설계 필요(범위 큼) |
| **T8** | UFB 코드표 전체 확정<br>(현재 미매핑 7종 포함) | A-2 | 기체+지상 | 코드값 확정 | **T7** |
| **T9** | 재전송 인덱스를 `Flags` 여유비트에 | RF 진단 | 기체+지상 | 없음(여유비트) | 선택, T3와 동시 권장 |
| **T10** | `LoadState()` 실패 경로 이벤트 추가 | 조용한 실패 | 기체 | 없음 | 없음 |
...
- **T1/T10/T6은 서로 독립이고 프로토콜 변경이 없어** 언제든 개별 착수 가능
