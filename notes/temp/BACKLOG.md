...
| **BL-10** | ⏸ **보류 확정(2026-07-24, 사용자 지시) — "viewpoint/짐벌은 추후 고려".** 재확인(2026-07-23): FC 실전송은 그전에도 보류였음(2026-06-08 명시적 범위 제외 유지). MAVLink 표준상 `DO_SET_ROI`/`CONDITION_YAW`/`DO_MOUNT_CONTROL`로 기술적으로 가능은 하나(짐벌 있으면 마운트 제어, 없으면 기체 자세만), **실제 짐벌 하드웨어 탑재 여부 미확인**. 대안 "스냅샷 발행"(캐시→`VIEWPOINT_SNAPSHOT_MID` 신규→lora_tdm_app 다운링크, ROUTE_SNAPSHOT_MID 0x1913과 동일 패턴)도 미확정 상태로 보류. **재개 조건**: 짐벌 하드웨어 탑재 여부 확인 후. 재개 시 결정 대상은 그대로 ① 짐벌 유무 ② FC 실행 vs 스냅샷 발행 | `ground_plan` P2 / `cfs_core_app_command_execution_gap` | 보류 — 짐벌 확인 시 재개 |
