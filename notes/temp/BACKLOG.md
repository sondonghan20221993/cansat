...
**base 배선(이 레포, 착수 대상)**: 비행모드 명령 3종 **HOVER/WAYPOINT/LAND** — 신규 command class(FLIGHT_MODE) → uplink_app 파싱 → 신규 MID → mavlink_bridge → FC(`DO_SET_MODE`, WAYPOINT는 `MISSION_SET_CURRENT` 동반). 2-pass는 지상이 이 base 명령들을 조합(waypoint→끝감지→HOVER→보정 재업로드→waypoint→…). 배선·파싱·단위테스트 선행 가능, **실외 원형비행 실기 검증만 하드웨어 대기**. 스코핑: `../route_2pass_gps_correction_scoping_2026-07-22_completed.md` | 지상 연산 + 3-모드 확정 |
