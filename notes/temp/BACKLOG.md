...
**mavlink_bridge_app 슬라이스 — wire-level spec 확정(2026-07-24), 구현 미착수**: spec §18.4.6.8.1 신설. ① SB 구조 정정: `UPLINK_APP_FlightModeCtrlCmd_t`에 `SourceSequence`(u16) 누락 발견·추가(EXEC_RESULT echo 필수 필드, uplink_app 쪽도 재수정 필요). ② PX4 custom_mode 인코딩(`(main<<16)|(sub<<24)`), COMMAND_LONG param1/2 값(base_mode=1.0f, custom_mode를 float로 수치 변환) 확정. ③ `MISSION_SET_CURRENT`(msg 41, seq/target_system/target_component, CRC_EXTRA=28) 프레임 신규 정의. ④ 결과판정 범위 명시: EXECUTED_OK=전송성공(FC ACK 대기 범위 밖, 기존 fire-and-forget 패턴과 동일). 실외 원형비행 실기 검증만 하드웨어 대기. 스코핑: `../route_2pass_gps_correction_scoping_2026-07-22_completed.md` | 지상 연산 + 3-모드 + PX4 매핑 확정 |
...
| **BL-48** | **최종 전수 실기 검증** (사용자 지시 2026-07-22): 전 CONFIG 파라미터(mavlink 7종+lora+cfs_core 6종) + RECOVERY 6종 + COUNTER 4종 + ROUTE + DIAGNOSTIC 전 명령, Pi EVS와 지상 UFB 양쪽 대조. 2026-07-23 부분 수행(CONFIG 1종/ROUTE/readback 체인) — 전수는 잔여. 부속: TEST_CASES.md RT-LORA-001 제외 문구 stale 정정(재편입), 0x1913 readback 파이프라인(DIAGNOSTIC→DL2 페이지→지상 재조립) 실기 왕복, uplinkGUI DIAGNOSTIC 버튼. **2026-07-24 추가**: 단위테스트만 있고 RT 케이스 미등재였던 BL-41/42/43/45를 `TEST_CASES.md`에 신규 등재(RT-CONFIG-001~003, RT-ROUTE-001~004, RT-TIMEBASE-001~002, RT-BOOTLOOP-001~003, RT-DL2-DEFAULT-001~002, 전부 ⬜ 미실행) — 전수 검증 시 이 목록도 포함 | 동 / `runtime_test_session_2026-07-23` |
| **BL-49** | 🔶 **PX4 미션 업로드 quirk 재검증 (2026-07-24 등재)**. `mavlink_bridge_app`의 미션 업로드 로직 일부(`MISSION_CLEAR_ALL` 필수 선행, `sysid=255` 요구, `MAV_FRAME_LOCAL_NED` 거부→`GLOBAL_RELATIVE_ALT` 강제)는 **ArduPilot 실측**(2026-05~06) 기반으로 작성됐으나 실제 연결 FC는 **PX4**(MicoAir743v2, 기체=헬리콥터/TradHeli 아닌 PX4 자체 헬리 지원)로 확인됨(2026-07-14, README/spec에 이미 플래그돼 있던 사실 — 이번 세션에 재확인). 코드를 지금 정리하면 PX4에서 실제로 어떻게 동작하는지 모르는 채 동작 변경이라 위험 — **실기(FC) 재검증 선행 필수**, 하드웨어 게이트. HEARTBEAT `autopilot` 필드 이중 인식(`3`=ArduPilot/`12`=PX4)은 잔재가 아니라 정상 방어 로직이라 변경 불필요(확인 완료, 유지). 카메라 `ardupilot_msp_osd.param`도 PX4 MSP OSD 파라미터 형식 확인 필요 — BL-36(카메라 P2/P4)에 포함. 근거: `mavlink_bridge_app_behavior_spec.md` §6 상단, `mavlink_bridge_app/README.md` | PX4 실기 재검증 필요, 하드웨어 대기 |
