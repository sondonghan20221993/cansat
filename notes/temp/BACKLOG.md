...
| **BL-63** | 🔴 **uplink_app LoRa 경로 auth flag 유실 — 실비행 CONFIG/ROUTE_UPDATE 항상 거부**. `tools/uplink_config_sender.py:208`(lora-serial 분기)는 `build_lora_frame()`에 `flags` 인자를 안 넘겨 기본값 0. `tools/uplink_route_update_sender.py`는 lora-text/lora-serial **두 분기 모두** `--auth`/`--force`로 계산한 `flags` 변수를 만들어놓고 LoRa 출력에 안 씀. UDP 분기만 정상이라 지금까지 안 잡힘. C 프로덕션 코드(spec §17.5/§18.4.7/§18.11)는 라인 단위로 정합 확인됨 — 버그는 전부 CLI 툴 쪽. 저수준 빌더 함수만 직접 호출하는 기존 테스트(`test_uplink_config_sender.py`/`test_uplink_route_update_sender.py`)는 버그 있는 `main()`의 transport 분기를 안 거쳐서 못 잡음, `test_uplink_e2e.py::test_valid_route_update_routed`도 `assert True # placeholder`라 실질 검증 없음. **수정**: 두 sender의 lora-serial(+route_update는 lora-text도) 분기에 계산된 flags 전달, placeholder 제거하고 실제 검증으로 교체, transport 분기별 유닛테스트 추가 | `spec_code_review_2026-07-27` §2 |
...
| **BL-65** | 🟡 **cfs_core_app RECOVERY RESET_COUNTER 비대칭**. `cfs_core_app_utils.c:1002-1010` — `RESET_COUNTER` 처리가 `RecoveryStartMs`/`BridgeRestartCount`만 리셋하고 `UplinkRestartCount`/`LoraRestartCount`/`NextUplinkRestartMs`/`NextLoraRestartMs`는 그대로 둠. BL-43에서 세 재시작 카운터를 대칭적으로 영속화하도록 설계했는데 지상 RESET_COUNTER 명령은 bridge 것만 지워 운영자 기대와 다른 동작. **수정 확정**: `RESET_COUNTER` 분기에 `BridgeRestartCount`뿐 아니라 `UplinkRestartCount`/`LoraRestartCount`/`NextBridgeRestartMs`/`NextUplinkRestartMs`/`NextLoraRestartMs` 전부 대칭 리셋 추가. **기존 UT 2건**(`Test_CFS_CORE_APP_ResetCounter_PersistsZero`, `Test_CFS_CORE_APP_ProcessRecoveryCommand_ResetCounter`) 둘 다 `BridgeRestartCount`만 검증 — 이 비대칭 버그를 못 잡는 상태 확인됨, 수정 시 Uplink/Lora 카운터 사전세팅+리셋검증 추가 필요 | `spec_code_review_2026-07-27` §3-2 |
