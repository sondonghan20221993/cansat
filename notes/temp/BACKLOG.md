...
| **BL-44** | 🔶 **아키텍처 확정(2026-07-24) — 지상국 연산 방식, cFS측 구현/실기 잔여**. 결정: 원 피팅·상태머신·1500샘플 버퍼를 온보드에서 **지상국(openMCT/Python, 별개 레포)으로 이관**(사용자 결정). 근거: 최소자승/상태머신은 C 비행SW에서 크고 테스트난이+안전민감, 지상 Python은 수 줄+사람 검토 가능, route/텔레메트리 경로 이미 존재. spec §18.4.6.2.1 개정 완료(책임 분담표, 링크 의존 폴백). **cFS측 최소 변경(이 레포, 미착수)**: ① lap1 위치 샘플 다운링크 충분성 확인(대체로 DL2 충족) ② lap 끝 LOITER_UNLIM 경로 ③ 호버링 시에만 armed 미션 업로드 허용 게이트 ④ 지상 명령→`MISSION_SET_CURRENT(0)` 전송. **전부 실물 FC+실외 원형비행 필요라 하드웨어 게이트**. 원 피팅·상태머신은 지상 레포 담당. **⚠️ 명시 등록(2026-07-24, 사용자 지시)**: 지상 산출물(보정 route·`MISSION_SET_CURRENT(0)`·LOITER_UNLIM 활성화)을 **uplink 명령 경로로 전달하는 배선**은 하드웨어 검증과 별개로 *지금 구현 가능한 코드 대상*이다 — 새 uplink 명령/필드(예: ROUTE_UPDATE reserved의 2-pass 플래그, MISSION_SET_CURRENT 전달용 명령 클래스/FcnCode) 정의와 uplink_app→FC 전달 배선을 착수 항목으로 등록. 실기 검증만 하드웨어 대기. 스코핑: `../route_2pass_gps_correction_scoping_2026-07-22_completed.md` | 지상 연산 확정 |
