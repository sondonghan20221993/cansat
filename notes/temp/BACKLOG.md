...
## ✅ BL-08 제외하고 지금 착수 가능한 것 (2026-07-21 정리)

`BL-08`(실행결과 회신 채널)은 3개 대상 앱의 결과 스키마가 제각각이라
설계에 시간이 걸림 — **보류**. 그에 종속된 `BL-05`/`BL-11`도 자동 보류.
아래는 그거 없이 지금 바로 갈 수 있는 항목만 추린 것.

```
🔴 즉시(독립, 프로토콜 무관)
   BL-01  재전송 중복 오탐 차단          ← 배포된 코드가 버그, 최우선
   BL-06  stale TODO 주석 2건 정정
   BL-07  cfs_core_app_command_execution_gap.md 문서 갱신
   BL-17  LoadState() 실패 시 이벤트 추가
   BL-18  SaveState()에 fsync() 추가
...
🟡 무선 프로토콜 한 번에 묶어서 (v2 확정 반영)
   BL-03  다운링크에 seq 동봉 (v2/DL2, v1은 제외 확정)
     ...
     ├─ BL-13 모듈러 seq 비교
     └─ BL-14 재전송 인덱스(선택)
     → 완료 시 BL-02 판단 자동 해소, BL-24(openMCT) 재검토

🟣 중간 규모, BL-08과 무관
   BL-09  cfs_core_app RECOVERY 명령 실제 연결 (+BL-25 함께)
   BL-26/27/28  매직넘버 C↔Python 교차검증 테스트로 묶기
