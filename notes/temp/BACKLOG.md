...
| **BL-66** | 📄 **mavlink_bridge_app_behavior_spec.md가 BL-56(2026-07-25) 리팩터링 미반영**. ① ARMED 차단 정책 정반대(spec: 차단, 실제: BL-56에서 전면 폐지) ② RouteOpType 체계 다름(spec 3종 REPLACE/APPEND/DELETE(count) vs 실제 4종 REPLACE/ADD/DELETE(index)/MODIFY) ③ 좌표 표현(spec: local NED→global 변환 서술, 실제: 절대좌표 LatE7/LonE7 그대로, 변환 로직 삭제됨) ④ §17 제목 "코드 미적용"은 오기(실제 이미 적용됨) ⑤ §15가 같은 문서 §7/§13.1.1과 자기모순(§7/§13.1.1이 맞음). 코드 자체 버그는 없음, 문서 전면 재작성 필요(§5/§5.1/§7/§13.1/§15/§17) | `spec_code_review_2026-07-27` §4 |
| **BL-67** | 📄 **cfs_core_app_behavior_spec.md 보강 필요**. ① `DIAGNOSTIC_CMD_MID`(0x1910)/`ROUTE_SNAPSHOT_MID`(0x1913) 코드엔 있는데 spec §6.1/§6.2/§16/§17 미기재 ② checksum 공식(§14.5)이 BL-43 추가 필드(BridgeRestartCount 등) 반영 안 된 옛 6필드 서술 ③ "PARSER_RESET/SERIAL_RECONNECT는 항상 OK" 서술이 부정확(`SendBridgeCtrlCmd()` 반환값 따라 FAILED 가능) ④ PublishPeriodMs 확장 시 재시작 감시 타이밍도 같이 지연되는 부작용 미기재 | `spec_code_review_2026-07-27` §3-1,3,4,5 |
| **BL-68** | 📄 **lora_protocol_v2_spec.md §4 UFB 표에 0x0C~0x0E 누락**. REJECT_COUNTER(0x0C, BL-CTR)/REJECT_FLIGHT_MODE(0x0D, BL-44)/UFB_APPLIED(0x0E, BL-55) 3개가 표에 안 실림. `lora_tdm_app_behavior_spec.md` §9.2는 최신 반영됨 — 두 spec 문서 간 불일치일 뿐, C/uplink_app enum 값 자체는 일치 | `spec_code_review_2026-07-27` §1 부차 |
