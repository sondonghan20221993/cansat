...
| **BL-62** | 🔵 **`MISSION_ITEM_REACHED`가 다운링크에 노출 안 됨(2026-07-26, BL-58① 구현 중 발견)**. BL-58① 2-pass 오케스트레이션은 원래 lap1 종료 판정을 FC가 발행하는 `MISSION_ITEM_REACHED`(마지막 waypoint seq)로 하기로 설계(§18.4.6.2.1)했는데, 실제로 `lora_protocol_v2.py`의 다운링크 디코딩에 이 이벤트가 전혀 없음(grep 0건) — `mavlink_bridge_app`이 FC로부터 이 MAVLink 메시지를 받아 다운링크로 지상에 노출하는 경로 자체가 없는 것으로 추정, 확인 필요. **임시 우회**: openMCT 쪽 2-pass 구현은 "마지막 웨이포인트 3m 이내 근접 + 최소 샘플 8개"라는 근접 프록시로 대체 완료(정확도 다소 떨어짐 — 마지막 지점 근처를 그냥 지나치기만 해도 트리거될 수 있음). **필요 작업**: `mavlink_bridge_app`이 FC MAVLink `MISSION_ITEM_REACHED`(msg #46)를 구독해 새 다운링크 필드/이벤트로 노출 → `lora_tdm_app` DL2 인코딩에 반영 → openMCT 디코딩 후 2-pass 프록시 로직을 실제 이벤트로 교체. firmware(cfs-telemetry-app) + openMCT 양쪽 작업 필요, 설계 미착수 | BL-58① 구현 중 발견 |
