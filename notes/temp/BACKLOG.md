...
| **BL-56** | 🔵 **설계 변경 필요 — mission upload ARMED 차단 제거(2026-07-24, 착수 전)**. 현재 `mavlink_bridge_app_utils.c:507`의 `IsArmed` 체크가 ROUTE_UPDATE(REPLACE/APPEND/DELETE) 전체를 비행 중(ARMED)엔 무조건 거부함(`ARMED_WARN_EID`) — 그 결과 REPLACE로 경로를 바꾸면 PX4가 현재 미션 인덱스를 리셋해 "다시 A로 날아가는" 문제가 생기고, 이를 우회하려면 FLIGHT_MODE(WAYPOINT, waypoint_start_index)로 재개 지점을 별도 지정해야 하는 번거로운 흐름이었음. 사용자 결정: "그냥 경로수정도 되게(비행 중에도) 하고 ARMED 차단(=force 없이도 막던 제한)을 제거하자" — 즉 IsArmed 가드를 없애고 APPEND/DELETE(REPLACE 대신) 위주로 기존 경로에 증분 수정이 되게 바꾸는 방향. 추가로 사용자 방향 확정(2026-07-24): 지금처럼 REPLACE(전체 스냅샷 덮어쓰기)를 기본으로 쓰지 말고, **ADD(단일/복수 웨이포인트 추가)·DELETE(인덱스 지정 삭제)·MODIFY(인덱스 지정 좌표 수정)** 형태의 증분 편집 위주 구조로 바꿀 것 — 현재 APPEND는 "끝에 붙이기"만 가능하고 특정 인덱스 삭제/수정은 아예 없음(DELETE는 "끝에서 N개 제거"만 가능, 임의 인덱스 삭제 불가). REPLACE는 위험한 전체교체 특수 케이스로만 남기고, 평소 경로 수정은 ADD/DELETE(by index)/MODIFY(by index) 3종으로 재설계 필요.

**미착수** — SDD(§18.4.6.2 route update, armed 가드 관련 서술 없음 확인됨) 갱신(op을 REPLACE/APPEND/DELETE 3종에서 ADD/DELETE(index)/MODIFY(index)/REPLACE 4종으로 재정의 포함) → 관련 회귀 테스트(`Test_StartMissionUpload_BlockedWhenArmed`, `Test_StartMissionUpload_Append`, `Test_StartMissionUpload_AppendTruncated` 등, `coveragetest_mavlink_bridge_app_utils.c:70~`) 재작성 → uplink_app/mavlink_bridge_app/GUI 3단 구현 → Pi 재검증 순서로 진행 필요 | 사용자 요청(openMCT GUI 워크스루 중 경로수정 흐름 논의) |
| **BL-57** | 🔵 **추후 설계 예정 — openMCT GUI 웨이포인트 지도 입력(Leaflet, 2026-07-24)**. 현재 Route Update GUI는 waypoint를 x/y/z 숫자 입력으로만 받는데(로컬 NED, 미터), 사용자가 이건 실사용성이 떨어진다고 판단 — 지도 위에서 클릭으로 웨이포인트를 찍는 방식으로 개선 희망. 방식은 3안(Leaflet 지도 라이브러리 + 오프라인 타일 로컬 서빙, 인터넷 없이 동작, GPS 위경도 기준 클릭) 확정, QGroundControl 등 기존 GCS 연동안(대안)은 기각. 핵심 설계 이슈로 논의 중 중단된 지점: **좌표계 변환**(지도 클릭의 lat/lon ↔ 기체가 쓰는 로컬 NED x/y/z 변환에 GPS origin 필요 — `mavlink_bridge_app`이 이미 갖고 있는 `MAVLINK_BRIDGE_APP_RefLatE7`/`RefLonE7` 필드 재사용 가능성 확인 필요), 오프라인 타일 확보/로컬 서빙 방식, BL-56(경로 증분 편집 ADD/DELETE/MODIFY)과의 통합 방식(지도에서 개별 포인트 추가/삭제/이동을 어떻게 ADD/DELETE(index)/MODIFY(index)에 매핑할지). 착수 전 — SDD 설계 논의부터 재개 필요 | 사용자 요청(openMCT Route Update GUI 사용성 논의) |
| **BL-58** | 🔵 **[openMCT 레포 종합] 지상국(ground station) 쪽 미착수 작업 총정리(2026-07-24)**. BL-44 완료 시 "원 피팅·상태머신은 지상국(openMCT/Python)으로 이관"이라 결정만 해두고, **openMCT 레포에 2-pass GPS 능동보정 로직(샘플 수집→원 피팅→보정 waypoint 계산→HOVER/REPLACE/WAYPOINT 오케스트레이션) 자체가 코드로 하나도 없음**(확인: circle_fit/correction 관련 코드 grep 결과 없음). 이걸 축으로 openMCT 쪽 미해결 항목을 한데 모음: ① **2-pass 오케스트레이션 신규 구현**(BL-44 잔여, 미착수 — 지상 파이썬 로직 설계부터) ② **BL-55**: ROUTE 세그먼트거리 위반 시 GUI UFB 오표시 버그 ③ **BL-56 GUI 절반**: route op이 REPLACE/APPEND/DELETE 3종에서 ADD/DELETE(index)/MODIFY(index)/REPLACE로 바뀌면 GUI 패널(`plugin.js` Route Update 섹션)도 맞춰 재작업 필요(기체측은 cfs-telemetry-app 레포 별도 작업) ④ **BL-57**: 웨이포인트 지도 입력(Leaflet). 논리적 순서: ③(GUI가 다루는 op 종류 확정) → ④(지도 클릭을 그 op에 매핑) → ①(지도+ADD/DELETE/MODIFY로 만든 경로를 2-pass가 소비) → ②(전 과정에서 GUI 피드백이 정확해야 하므로 언제든 병행 가능, 독립). 착수 전 — 실제 설계 논의부터 재개 필요 | 사용자 요청(openMCT 잔여작업 총정리) |
