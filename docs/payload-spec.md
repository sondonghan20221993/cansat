...
전송률을 **4KB/s로 상향**하면 마진을 크게 늘릴 수 있음(아래 비교 참조) — 하드웨어가
이미 지원하는 옵션이라 계산 마진을 빡빡하게 쓰기보다 우선 검토 대상. 단, 레이트 상향이
실제 통신거리(스펙상 10km)를 줄이는 트레이드오프가 있는지는 실측 필요.
...
`cfs-telemetry-app` BL-15 Stage 4b (2026-07-22, 5분 soak, 실기체) 실측 결과:

| 항목 | 값 |
| --- | --- |
| 설정 | `CYCLE_PERIOD_MS=100`(10Hz), `RX_WINDOW_MS=50` |
| DL2 프레임 크기 | 58B (SysTime 포함), 에어타임 ~19ms |
| 손실률 | **0.00%** (2991/2991, gap 이벤트 0회) |
| 실효 수신율 | 9.97 pkt/s (이론 10/s) |
| 대역폭 사용 | 58B×10Hz ≈ **580 B/s**, 명목 2400 B/s 대비 **24% 사용, 마진 76%** |

| | 구 DL2 (실측, 2.4KB/s) | 신규 MAVLink 10Hz안 (계산, 2.4KB/s) | 신규 MAVLink 10Hz안 (계산, 4KB/s 상향 시) |
| --- | --- | --- | --- |
| 대역폭 사용 | 580 B/s | ≈1469 B/s | ≈1469 B/s |
| 예산 대비 사용률 | 24% (2400B/s 기준) | 61% (2400B/s 기준) | 37% (4000B/s 기준) |
| 마진 | 76% | 39% | 63% |

LR24-F를 4KB/s로 설정하면 마진이 이전 DL2 실측(76%)에 근접한 63%까지 회복된다 —
계산 마진이 부족하면 레이트 조합을 줄이기보다 이 옵션을 우선 검토.
