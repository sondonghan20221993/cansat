...
측정치:
    ...
    - **통합** seq gap 기반 손실률 (RF 링크 품질 지표)
    - FC 데이터 신선도 (boot_ms 정체 감지 — FC UART 문제 지표, loss와 다른 축)
    - link_state / uplink_fb 분포

정정 이력(2026-07-13): 이전 버전은 source(FC/SH)별로 seq gap을 나눠 계산했으나,
lora_tdm_app은 FC/SH가 **단일 공유 카운터(DownlinkSeq)를 짝/홀로 교대**하는 설계라
(RunTx, fc_serial_ws_server.py 상단 주석 참조) source별로 쪼개면 정상 링크도 항상
~50% 손실로 오판된다. RF 링크 품질은 반드시 통합 seq로 판단해야 한다.
FC 쪽 문제(UART 끊김 등)는 loss로 드러나지 않는다 — lora_tdm은 FC가 죽어도 캐시에
남은 값으로 계속 프레임을 짜서 보내므로, 대신 boot_ms(FC TimestampMs) 정체로 진단한다.

판정 기준(runbook과 동일 수치, notes/lora_stage_measurement_runbook.md 참조):
    Stage 1: 통합 손실률 < 5%, ACK 송신 후 기체 link_state 값 관찰(정상화 여부는 기체 HK로 별도 확인)
...
def analyze(path: str, cycle_ms: int) -> int:
    ...
    # 통합 seq gap 손실률 (RF 링크 품질) — FC/SH 섞어서 계산해야 정확함.
    # DownlinkSeq는 lora_tdm_app에서 짝수=FC/홀수=SH로 교대하는 단일 카운터라,
    # source별로 나누면 정상 gap(2)이 항상 손실로 잡힌다 (2026-07-13 정정).
    all_seqs = [int(r["seq"]) for r in rows if r.get("seq", "").strip().lstrip("-").isdigit()]
    if len(all_seqs) >= 2:
        gaps = [b - a for a, b in zip(all_seqs, all_seqs[1:]) if b > a]
        expected = sum(gaps) if gaps else 0
        received = len(gaps)
        loss_pct = round((expected - received) / expected * 100, 2) if expected > 0 else 0.0
        print(f"\n통합 seq gap 손실률(RF 링크 품질): {loss_pct}% (expected={expected}, received={received})")
    else:
        loss_pct = None
        print("\n통합 seq gap 손실률: 계산 불가 (seq 2건 미만)")

    # FC 데이터 신선도 — boot_ms(FC TimestampMs) 정체 구간 감지. FC UART가 끊겨도
    # lora_tdm은 캐시값으로 프레임을 계속 보내 loss에는 안 잡히므로 별도 확인 필요.
    fc_rows = [r for r in rows if r.get("source") == "FC" and r.get("boot_ms", "").strip().isdigit()]
    if len(fc_rows) >= 2:
        boot_ms_vals = [int(r["boot_ms"]) for r in fc_rows]
        stale_streak = 0
        max_stale_streak = 0
        for a, b in zip(boot_ms_vals, boot_ms_vals[1:]):
            if b == a:
                stale_streak += 1
                max_stale_streak = max(max_stale_streak, stale_streak)
            else:
                stale_streak = 0
        if max_stale_streak > 0:
            print(f"⚠ FC boot_ms 정체: 최대 연속 {max_stale_streak}회 동일값 "
                  f"(FC UART 끊김/캐시값 재전송 의심 — loss와 무관한 별개 문제)")
        else:
            print("FC boot_ms: 매 프레임 갱신됨 (정체 없음)")

    # source별 집계 (품질/손실 판단용 아님 — 참고 카운트 및 다른 필드 분포만)
    by_source = defaultdict(list)
    for r in rows:
        by_source[r.get("source", "?")].append(r)

    for source, srows in sorted(by_source.items()):
        print(f"\n=== source={source} ({len(srows)}건, 참고용 — 손실률은 위 통합 수치를 볼 것) ===")
