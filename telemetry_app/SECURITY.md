...
`telemetry_app` 하위 시스템의 취약점을 보고하려면 [이슈를 생성](https://github.com/nasa/telemetry_app/issues/new/choose)한다.
...
**면책 고지: nasa/telemetry_app은 [Apache License 2.0](https://github.com/nasa/telemetry_app/blob/main/LICENSE)에 따라 발생하는 책임을 부담하지 않는다.**

테스트는 `telemetry_app` 품질 향상을 위해 중요한 활동이다.

cFS 번들에 사용되는 도구는 [상위 보안 정책](https://github.com/nasa/cFS/security/policy)에서 확인할 수 있다.

### CodeQL

[telemetry_app CodeQL GitHub Actions 워크플로](https://github.com/nasa/telemetry_app/actions/workflows/codeql-build.yml)는 공개되어 있다. 결과를 검토하려면 `telemetry_app` 저장소를 포크하고 CodeQL 워크플로를 실행한다.

CodeQL은 GitHub Actions에서 `telemetry_app`의 모든 브랜치에 대한 push와 pull request마다 실행된다.

CodeQL GitHub Actions 구성은 <https://github.com/github/codeql-action>에서 확인할 수 있다.

### Cppcheck

[telemetry_app Cppcheck GitHub Actions 워크플로와 결과](https://github.com/nasa/telemetry_app/actions/workflows/static-analysis.yml)는 공개되어 있다. 결과를 확인하려면 해당 워크플로를 선택하고 artifact를 다운로드한다.

Cppcheck는 GitHub Actions에서 `telemetry_app`의 main 브랜치 push와 모든 pull request마다 실행된다.
